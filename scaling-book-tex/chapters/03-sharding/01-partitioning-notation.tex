...
\CommentTok{\# Create our mesh! We\textquotesingle{}re running on a TPU v2{-}8}
\CommentTok{\# 4x2 slice with names \textquotesingle{}X\textquotesingle{} and \textquotesingle{}Y\textquotesingle{}.}
...
\CommentTok{\# A little utility function to help define our sharding.}
\CommentTok{\# A PartitionSpec is our sharding (a mapping from axes}
\CommentTok{\# to names).}
\KeywordTok{def} \FunctionTok{P}\NormalTok{(}\OperatorTok{*}\NormalTok{args):}
  \ControlFlowTok{return}\NormalTok{ jax.NamedSharding(}
\NormalTok{      mesh, jax.sharding.PartitionSpec(}\OperatorTok{*}\NormalTok{args))}

\CommentTok{\# We shard both A and B over the non{-}contracting}
\CommentTok{\# dimension and A over the contracting dim.}
\NormalTok{A }\OperatorTok{=}\NormalTok{ jnp.zeros(}
\NormalTok{    (}\DecValTok{8}\NormalTok{, }\DecValTok{2048}\NormalTok{), dtype}\OperatorTok{=}\NormalTok{jnp.bfloat16,}
...
\NormalTok{    (}\DecValTok{2048}\NormalTok{, }\DecValTok{8192}\NormalTok{), dtype}\OperatorTok{=}\NormalTok{jnp.bfloat16,}
...
\CommentTok{\# We can perform a matmul on these sharded arrays!}
\CommentTok{\# out\_shardings tells us how we want the output}
\CommentTok{\# to be sharded. JAX/XLA handles the rest of the}
\CommentTok{\# sharding for us.}
\NormalTok{y }\OperatorTok{=}\NormalTok{ jax.jit(}
\NormalTok{    }\KeywordTok{lambda}\NormalTok{ A, B: jnp.einsum(}\StringTok{\textquotesingle{}BD,DF{-}\textgreater{}BF\textquotesingle{}}\NormalTok{, A, B),}
