...
\begin{takeawaybox}
\textbf{Takeaway:} Tensor parallelism over an axis of size Y with feed-forward dimension F becomes bandwidth-bound when the $Y > F / 2475$, which generally constrains us to only intra-node TP or at most 2-node TP.
\end{takeawaybox}
...
\begin{takeawaybox}
\textbf{Takeaway:} if $F < 8 * C / W_\text{node}$, expert parallelism can span 1-2 nodes with similar (slightly lower) cost to TP, or if $F > 8 * C / W_\text{node}$, we can do a significant amount of expert parallelism (up to $E$ nodes) with relatively low cost.
\end{takeawaybox}
