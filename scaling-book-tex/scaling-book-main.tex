...
% Using balanced margins to avoid overflow feeling
\usepackage[
    b6paper,
    inner=1.6cm,      % Binding side margin (more space for binding)
    outer=0.8cm,      % Outer edge margin (reduced for more text width)
    top=0.2cm,        % Top margin for breathing room
    bottom=1.0cm,     % Bottom margin for breathing room
    includeheadfoot
]{geometry}
...
% Essential packages
\usepackage{amsmath}
% Note: amssymb is loaded by fontsetup, so we don't load it again to avoid conflicts
...
% Load framed package first
\usepackage{framed}

% Redefine snugshade environment (framed package provides a default version)
\makeatletter
...
% Custom symbols for tables (checkmark and cross mark)
...
\fancyfoot[C]{\small\rmfamily\thepage} % Center page number in footer with smaller Roman font
...
\input{chapters/08-applied-inference}
\input{chapters/09-profiling}
\input{chapters/10-jax-stuff}
\input{chapters/11-conclusion}
\input{chapters/12-gpus}
